...
class Attachment extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['company_id','name','description','file_path','remark'], 'string', 'max' => 255],
            [['status'], 'safe'],
    ...
    {
        return $this->hasMany(\base\models\AttachmentAccess::className(),['attachment_id' => 'id']);;
    }
    public function getAttachmentMetatags()
    {
        return $this->hasMany(\base\models\AttachmentAccess::className(),['attachment_id' => 'id'],\base\models\AttachmentAccess::className(),['viewable_by' => 'id'],\base\models\AttachmentAccess::className(),['downloadable_by' => 'id'],\base\models\AttachmentMetatag::className(),['attachment_id' => 'id']);;
    }

    public static function arrayList($callback = false)
    {
        $callback = is_array($callback) ? $callback : self::find()->orderBy('attachment.id')->asArray()->all();
        return \yii\helpers\ArrayHelper::map($callback, 'id', 'attachment.name');
    }
    ...
    {
        $query = new \backend\models\query\AttachmentQuery(get_called_class());
        ...
        return $query->andWhere(['attachment.deleted_by' => 0]);
    }
